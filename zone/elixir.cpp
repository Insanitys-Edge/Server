...
int8 Mob::ElixirCastSpellCheck(uint16 spellID, Mob** outMob)
{
	...
	int healPercent = 90;
	if (IsNPC()) healPercent = RuleI(NPC, NPCsElixirHealPercent);
	...
	int aeMinimum = 3;
	if (IsNPC()) aeMinimum = RuleI(NPC, NPCsElixirAEMinimum);
	...
	bool isDebuff = false;
	bool isBuff = false;
	bool isLifetap = IsLifetapSpell(spellID);
	bool isMana = false;
	bool isCharm = false;
	bool isSnare = false;
	bool isSow = false;
	...
	for (int i = 0; i < EFFECT_COUNT; i++) {
		...
		if (attr == SE_Invisibility) { //12 Invisiblity
			isNoncombatSpell = true;
		}
...
//		if (spDat.components[i] < 1) continue;
//		if (spDat.component_counts[i] < 1) continue;
...
//	}
	...
	if (skillID == EQ::skills::SkillBackstab) { // 8 backstab
		...
		if (IsDetrimentalSpell(spellID) && !IsAttackAllowed(target)) {
	...
	else if (skillID && targettype == ST_Target) { // all other skills
		if (target == nullptr) {
			return ELIXIR_NO_TARGET;
		}
	...
	if (spDat.can_cast_in_combat == false) {
		if (IsEngaged()) {
			return ELIXIR_CANNOT_USE_IN_COMBAT;
		}
		if (target == nullptr) {
	...
	if (isCloneSummon)
	{
		...
		if (!target->IsAttackAllowed(this))
		{
			return ELIXIR_NOT_NEEDED;
		}
		...
		if(GetPet())
		{
			if (GetPet()->CastToNPC()->GetPetSpellID() == spellID && spellID != 2813)
				returnearly = true;
		}

		if (returnearly)
		{
			return ELIXIR_ALREADY_HAVE_PET;
		}
	...
	if (isPetSummon)
	{
		if(GetPet())
		{
			if (GetPet()->CastToNPC()->GetPetSpellID() == spellID && spellID != 43300)
				returnearly = true;
			else if (GetPet()->CastToNPC()->GetPetSpellID() == 43300)
				pet_spell_counts++;
		}

		if (spellID == 43300 && pet_spell_counts >= 2)
			return ELIXIR_ALREADY_HAVE_PET;
	...
	if (isHeal && isBeneficial) { //heal logic
