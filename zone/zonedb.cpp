...
bool ZoneDatabase::GetZoneCFG(
	...
	*map_filename = new char[100];
	...
	zone_data->expansion = atoi(row[65]);
...
Trader_Struct* ZoneDatabase::LoadTraderItem(uint32 char_id)
{
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		if (atoi(row[5]) >= 100 || atoi(row[4]) < 0) {
...
TraderCharges_Struct* ZoneDatabase::LoadTraderItemWithCharges(uint32 char_id)
{
	...
	std::string query = StringFormat("SELECT char_id, item_id, serialnumber, charges, item_cost, slot_id, aug1, aug2, aug3, aug4, aug5, aug6 FROM trader WHERE char_id=%i ORDER BY slot_id LIMIT 100", char_id);
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		LogTrading("Failed to load trader information!\n");
		return loadti;
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		if (atoi(row[5]) >= 100 || atoi(row[5]) < 0) {
			LogTrading("Bad Slot number when trying to load trader information!\n");
			continue;
		}
		...
		loadti->ItemCost[atoi(row[5])] = atoi(row[4]);
		loadti->AugID1[atoi(row[5])] = atoi(row[6]);
		loadti->AugID2[atoi(row[5])] = atoi(row[7]);
		loadti->AugID3[atoi(row[5])] = atoi(row[8]);
		loadti->AugID4[atoi(row[5])] = atoi(row[9]);
		loadti->AugID5[atoi(row[5])] = atoi(row[10]);
		loadti->AugID6[atoi(row[5])] = atoi(row[11]);
	}
	return loadti;
}

EQ::ItemInstance* ZoneDatabase::LoadSingleTraderItem(uint32 CharID, int SerialNumber) {
	std::string query = StringFormat("SELECT char_id, item_id, serialnumber, charges, item_cost, slot_id, aug1, aug2, aug3, aug4, aug5, aug6 FROM trader WHERE char_id = %i AND serialnumber = %i "
	...
	inst->PutAugment(&database, 0, atoi(row[6]) );
	inst->PutAugment(&database, 1, atoi(row[7]) );
	inst->PutAugment(&database, 2, atoi(row[8]) );
	inst->PutAugment(&database, 3, atoi(row[9]) );
	inst->PutAugment(&database, 4, atoi(row[10]) );
	inst->PutAugment(&database, 5, atoi(row[11]) );
	if(inst->IsStackable())
...
bool ZoneDatabase::LoadCharacterSpellBook(uint32 character_id, uint32 class_id, PlayerProfile_Struct* pp){
	std::string query = StringFormat(
		"SELECT					"
		"slot_id,				"
		"`spell_id`				"
...
bool ZoneDatabase::LoadCharacterDisciplines(uint32 character_id, uint32 class_id, PlayerProfile_Struct* pp){
	std::string query = StringFormat(
		"SELECT				  "
		"disc_id			  "
		"FROM				  "
...
bool ZoneDatabase::LoadCharacterCurrency(uint32 account_id, PlayerProfile_Struct* pp){
	std::string query = StringFormat(
		"SELECT                  "
		"platinum,               "
		"gold,                   "
...
bool ZoneDatabase::SaveCharacterDisc(uint32 character_id, uint32 class_id, uint32 slot_id, uint32 disc_id){
	std::string query = StringFormat("REPLACE INTO `character_disciplines` (id, class_id, slot_id, disc_id) VALUES (%u, %u, %u, %u)", character_id, class_id, slot_id, disc_id);
	auto results = QueryDatabase(query);
...
bool ZoneDatabase::SaveCharacterCurrency(uint32 account_id, PlayerProfile_Struct* pp){
	...
	std::string query = StringFormat(
		"REPLACE INTO `account_currency` (id, platinum, gold, silver, copper,"
	...
	auto results = database.QueryDatabase(query);
	LogDebug("Saving Currency for character ID: [{}], done", account_id);
	return true;
}
...
bool ZoneDatabase::SaveCharacterLootLockout(uint32 account_id, uint32 expiry, uint32 npctype_id, bool aa_only)
{
	auto zone_id_number = zone ? zone->GetZoneID() : 0;

	std::string query = StringFormat("REPLACE INTO `server_lockouts` (account_id, expiry, npctype_id, aa_only) VALUES (%u, %u, %u, %u)", account_id, expiry, npctype_id, aa_only == true ? 1 : 0);
	auto results = QueryDatabase(query);
	return true;
}
...
std::map<uint32, MercCharacter_Struct*> ZoneDatabase::LoadCharactersOnAccount(Client* c,
	uint32 AccID, uint32 InitiatorCharacterID)
{
	...
	std::string query = StringFormat(
		"SELECT `id` FROM `character_data` WHERE `id` = '%d' AND `deleted_at` IS NULL LIMIT 1", InitiatorCharacterID);
	...
	for (i = 1; i <= 15; i++)
	{
		...
		bool fail = !database.GetInventory(charid, AccID, i, &charStruct->m_inv); /* Load Character Inventory */
		if (fail)
		{
		...
		database.LoadCharacterDisciplines(charid, i, &charStruct->m_pp); /* Load Character Spell Book */
