		...
		for (uint32 j = 0; j < lds->NumEntries; ++j) {
			...
			if (db_item) {
				if (roll < lds->Entries[j].chance) {
					...
					const EQ::ItemData* dbitem = GetItem(lds->Entries[j].item_id);
					if (dbitem == nullptr)
						continue;
...
void ZoneDatabase::DisplayFullLootDropList(Client* c, uint32 lootdrop_id) {
	...
	for (uint32 i = 0; i < lds->NumEntries; ++i) {
		...
		c->Message(0, "| -- Item: %i - (%s), Charges: (%i), Chance: (%i), Will Equip?: (%s), Multiplier: (%i),  Min/Max Level: (%i / %i)",
			...
			lds->Entries[i].npc_min_level,
			lds->Entries[i].npc_max_level);
		//}
		//}
	}
	return;
			...
			for(int j = 0; j < charges; ++j) {
				if(zone->random.Real(0.0, 100.0) <= lds->Entries[i].chance) {
					...
					if(!dbitem)
						continue;
					bool force_equip = lds->Entries[i].equip_item == 2;
					npc->AddLootDrop(dbitem, itemlist, lds->Entries[i].item_charges, lds->Entries[i].npc_min_level,
									lds->Entries[i].npc_max_level, lds->Entries[i].equip_item > 0 ? true : false, false, false, false, force_equip, 0,0,0,0,0,0);
				}
			}
		}
		return;
	...
	for(int i = 0; i < mindrop; ++i) {
		...
		for(uint32 j = 0; j < lds->NumEntries; ++j) {
			...
			if(db_item) {
				if(roll < lds->Entries[j].chance) {
					bool force_equip = lds->Entries[i].equip_item == 2;
					npc->AddLootDrop(db_item, itemlist, lds->Entries[j].item_charges, lds->Entries[j].npc_min_level,
									 lds->Entries[j].npc_max_level, lds->Entries[j].equip_item > 0 ? true : false, false, false, false, force_equip, 0, 0, 0, 0, 0, 0);
				...
				if(roll < lds->Entries[j].chance) {
					...
					for(int k = 1; k < charges; ++k) {
						...
						if(c_roll <= lds->Entries[i].chance) {
							bool force_equip = lds->Entries[i].equip_item == 2;
							npc->AddLootDrop(db_item, itemlist, lds->Entries[j].item_charges, lds->Entries[j].npc_min_level,
											lds->Entries[j].npc_max_level, lds->Entries[j].equip_item > 0 ? true : false, false, false, force_equip, 0, 0, 0, 0, 0, 0, 0);
						}
					}
				...
				if (item2->Slots & slots) {
					if(equipment[i])
					{
						...
						if (item->pet == 0 && (i != EQ::invslot::slotPrimary && i != EQ::invslot::slotSecondary && i != EQ::invslot::slotRange && (item2->AC > compitem->AC || (item2->AC == compitem->AC && item2->HP > compitem->HP))) ||
							((i == EQ::invslot::slotPrimary || i == EQ::invslot::slotSecondary || i == EQ::invslot::slotRange) && item2->Damage > compitem->Damage) ||
							force_equip
							)
						{
...
bool NPC::AddQuestLoot(uint32 itemid, int8 charges)
{
	...
	if (item)
	{
		...
		AddLootDrop(item, &itemlist, charges, 0, 255, false, false, true,false,false);
...
bool NPC::AddPetLoot(uint32 itemid, int8 charges, bool fromquest)
{
	...
	if (!fromquest || valid)
	{
		if (item->ID)
		{
			AddLootDrop(item, &itemlist, charges, 0, 255, true, true, false, true,false,0,0,0,0,0,0);
...
int32 NPC::GetEquipmentMaterial(uint8 material_slot)
{
