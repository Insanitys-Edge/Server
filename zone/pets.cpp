	...
	if (content_db.GetBasePetItems(record.equipmentset, petinv)) {
		for (int i = EQ::invslot::EQUIPMENT_BEGIN; i <= EQ::invslot::EQUIPMENT_END; i++)
			if (petinv[i]) {
				...
				npc->AddLootDrop(item, &npc->itemlist, item->MaxCharges, 0, 127, true, true);
			}
	}
...
Mob* Mob::MakePoweredNPCPet(uint16 spell_id, uint32 npcTypeId, const char* petType, int16 petpower, float in_size) {
	// Sanity and early out checking first.
	if (npcTypeId == 0)
		return nullptr;
	...
	if (npcclass)
	{
		npc_type->class_ = npcclass->class_;
		if(npcclass->npc_spells_id < 21)
			npc_type->npc_spells_id = npcclass->class_;
	}
		strcpy(npc_type->name, this->GetCleanName());
		npc_type->name[25] = '\0';
		strcat(npc_type->name, "`s_animated_ally");

	npc_type->npc_faction_id = 0;
	memset(npc_type->special_abilities, 0, 512);
	...
	//Once our pet's stats are established, recalc our own bonuses to make sure the pet stays scaled across multiple pets.
	CalcBonuses();
	return npc;
}
	...
	for (i = EQ::invslot::EQUIPMENT_BEGIN; i <= EQ::invslot::EQUIPMENT_END; i++) {
		...
		if (item2) {
			...
			if (!noDrop || petCanHaveNoDrop) {
				AddLootDrop(item2, &itemlist, item2->MaxCharges,0,127,true,true,false,true,false,0,0,0,0,0,0);
			}
		}
	}
}
