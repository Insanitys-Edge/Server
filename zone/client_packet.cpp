...
void Client::Handle_Connect_OP_ZoneEntry(const EQApplicationPacket *app)
{
	...
	*/
	...
	loaditems = database.GetInventory(cid, AccountID(), m_pp.class_, &m_inv); /* Load Character Inventory */
	...
	if (m_petinfo.SpellID > 1 && !GetPet() && m_petinfo.SpellID <= SPDAT_RECORDS) {
		if (spells[m_petinfo.SpellID].classes[m_pp.class_ - 1] != 255)
		{
...
void Client::Handle_OP_Consider(const EQApplicationPacket *app)
{
	...
	if (tmob && tmob->IsNPC() && tmob->CastToNPC()->GetFlagItem() != 0)
	{
		...
		if (LootLockoutItr == loot_lockouts.end() || !LootLockoutItr->second.HasLockout(curTime))
		{
			...
			{
				SendColoredText(Chat::System, "You are eligible for AA and loot upon looting the creature's corpse.");
			}
		}
		else
		{
			...
			{
				SendColoredText(Chat::System, fmt::format(
					"This creature grants AA and loot, but you are locked out of this NPC for {}.",
					time_string
				).c_str());
			}
		}
...
void Client::Handle_OP_DeleteSpell(const EQApplicationPacket *app)
{
	...
	if (m_pp.spell_book[dss->spell_slot] != SPELLBOOK_UNKNOWN) {
		...
		database.DeleteCharacterSpell(CharacterID(), m_pp.class_, m_pp.spell_book[dss->spell_slot], dss->spell_slot);
		dss->success = 1;
	}
	else
		dss->success = 0;
...
void Client::Handle_OP_SwapSpell(const EQApplicationPacket *app)
{
	...
	/* Save Spell Swaps */
	if (!database.SaveCharacterSpell(CharacterID(), m_pp.class_, m_pp.spell_book[swapspell->from_slot], swapspell->from_slot)) {
		database.DeleteCharacterSpell(CharacterID(), m_pp.class_, m_pp.spell_book[swapspell->from_slot], swapspell->from_slot);
	}
	if (!database.SaveCharacterSpell(CharacterID(), m_pp.class_, swapspelltemp, swapspell->to_slot)) {
		database.DeleteCharacterSpell(CharacterID(), m_pp.class_, swapspelltemp, swapspell->to_slot);
	}
		...
		if (ints->Code == BazaarTrader_StartTraderMode)
		{
			...
			for (uint32 i = 0; i < max_items; i++) {
				...
				if (inst && database.GetItem(inst->GetItem()->ID ? inst->GetItem()->ID : 0))
				{
					...
					database.SaveTraderItem(CharacterID(), inst->GetItem()->ID, inst->GetSerialNumber(), inst->GetMerchantCount(), ints->ItemCost[i], index, inst->GetAugmentItemID(0), inst->GetAugmentItemID(1), inst->GetAugmentItemID(2), inst->GetAugmentItemID(3), inst->GetAugmentItemID(4), inst->GetAugmentItemID(5));
					index++;
				}
				else {
