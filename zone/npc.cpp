...
int64 NPC::GetHPRegen()
{
	...
	if ((GetHP() < GetMaxHP()) && !IsPet())
	{
		...
		if (!IsEngaged())
		{
			return(GetOOCNPCHPRegen() + bonus); // hp_regen + spell/item regen + sitting bonus
		// In Combat
		}
		else
			return(GetNPCHPRegen()); // combat_regen + spell/item regen
	}
	// Pet
	else if (GetHP() < GetMaxHP() && GetOwnerID() != 0)
	{
		if (!IsEngaged() && GetPetType() != petCharmed)
			return(GetOOCNPCHPRegen() + bonus + (GetLevel() / 5));
		else
			return(GetNPCHPRegen());
	}
	else
		return 0;
}
...
int64 NPC::GetManaRegen()
{
	...
	if ((GetMana() < GetMaxMana()) && !IsPet())
	{
		...
		if (!IsEngaged())
		{
			return(GetOOCNPCManaRegen() + bonus); // mana_regen + spell/item regen + sitting bonus
		// In Combat
		}
		else
...
void NPC::Depop(bool StartSpawnTimer) {
	...
	if(emoteid != 0)
		DoNPCEmote(ONDESPAWN,emoteid,this);
	p_depop = true;
	if (respawn2)
	{
		if (StartSpawnTimer) {
